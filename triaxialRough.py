...
from FEMxDEM.msFEM3D import MultiScale
...
useml = False
nx, ny, nz = 2, 2, 4
loadingPath = 'TriaxialRough'
loadInfor = ('../simu/ML_%s' % mode if useml else '../simu/00DEM') + '_' + str(nx) + '_' + str(
    ny) + '_' + loadingPath
...
mydomain = Brick(l0=lx, l1=ly, l2=lz, n0=nx, n1=ny, n2=nz, order=2,
...
prob = MultiScale(domain=mydomain, ng=numg, useMPI=True, rtol=1e-2, mpi_pool=mpi_pool)  # mpi is activated
...
fout = open(os.path.join(loadInfor, 'resultant.dat'), 'w')
...
Nbc = whereZero(bx[0]) * [-confining, 0, 0] +\
      whereZero(bx[0] - sup(bx[0])) * [confining, 0, 0] + \
      whereZero(bx[1]) * [0,-confining,0] + \
...
while t < 100:
    ...
    saveGauss3D(os.path.join(loadInfor, 'gauss/time_' + str(t) + '.dat'), strain=strain, stress=stress, fabric=fabric)
    volume_strain = trace(strain)
    dev_strain = symmetric(strain) - volume_strain * k / dim
    shear = sqrt(2. / 3. * inner(dev_strain, dev_strain))
    saveVTK(os.path.join(loadInfor, "vtk/triaxialRough_%d.vtu" % t), disp=disp, shear=shear, e=vR, rot=rotation)
