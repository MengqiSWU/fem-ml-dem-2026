...
class ImplicitSolver(escriptSolver):
    ...
    def solve(self, t, iter_max):
        ...
        sig_data, D_data, S_data = self.stressSolver_3d_accum(deps=deps)  # 更新高斯点应力和对应的stiffness矩阵
        # self.H_3d += self.setHistVector_3D(scenes[1])

        iterate = 0
        if self.save_loading_flag:
            self.save_loading_mask(t=t, sig_data=sig_data, D_data=D_data, u_grad=deps, s_data = S_data,iterate=iterate)
        end_flag = False
        while True:
            ...
            # d_deps = symmetric(grad(du))
            sig_data, D_data, S_data = self.stressSolver_3d_accum(deps=deps)
            # self.H_3d += self.setHistVector_3D(scenes[1])
            if self.save_loading_flag:
                self.save_loading_mask(t=t, sig_data=sig_data, D_data=D_data, u_grad=deps, s_data=S_data, iterate=iterate)
            iterate += 1
        ...
        # self.eps_last = self.eps
        self.volume = self.volume*(1+trace(deps))
        self.updateCons(s_data=S_data)

    def save_loading_mask(self, t, sig_data, D_data, u_grad, s_data, iterate=None):   # du_grad,
        save_dict = {
            'sig': sig_data,
            'D': D_data,
            ...
            'H_3D': self.setHistVector_3D(s_data[1]),
        }
