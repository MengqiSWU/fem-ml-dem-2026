...
class MultiScale(object):
    ...
    def __init__(self, domain, pore0=0., perm=1.e-5, kf=2.2e9, dt=0.001, ng=1, useMPI=False, np=1, rtol=1.e-2, mpipool=None):
        """
    ...
    def initialize(self, b=escript.Data(), f=escript.Data(), umsk=escript.Data(), uvalue=escript.Data(),
                   flux=escript.Data(), pmsk=escript.Data(), pvalue=escript.Data()):
        """
    ...
    def solveSolid(self, p_iter_gauss=escript.Data(), iter_max=50):
        ...
        while (not converged) and (iterate < iter_max):
            ...
            converged = (err < rtol)
            print(
                '\t\tSolid: Converge !' if converged else '\t\tSolid: Not converge, err=%e' % err + 'iter %d' % iterate)
    ...
    def solve(self, globalIter=10, solidIter=50):
        ...
        kf = self.__bulkFluid
        dt = self.__dt
        self.__ppde.setValue(A=perm, D=(n / kf + 1. / kdr) / dt,
                             Y=(n / kf + 1. / kdr) / dt * self.__pgauss - self.__meanStressRate / kdr)
        p_iter_old = self.__ppde.getSolution()
        p_iter_gauss = util.interpolate(p_iter_old, escript.Function(self.__domain))
        print('\t' + '-' * 20 + 'Solve iter %d' % 0 + '-' * 20)
        ...
        while (not converge) and (iterate < globalIter):
            ...
            print('\t' + '-' * 20 + 'Solve iter %d' % iterate + '-' * 20)
            ...
            print(
                '\tSolve: Converge !' if converge else '\tSolve: Not converge, u_err=%e p_err=%e' % (
                u_err, p_err) + ' iter %d' % iterate)
