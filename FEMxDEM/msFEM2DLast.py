...
from esys.escript import util, Vector, Solution, trace
...
import numpy as np
from FEMxML.convertStrain import rotation, rotateStrain, rotateStress
...
class MultiScale(object):
    ...
    def __init__(self, domain, ng=1, useMPI=False, np=1, random=False, rtol=1e-2, usePert=False, pert=-2.e-6,
                 verbose=True, useml=False, loadInfor=None, mode='net', ml_model_path=None, saved_model=None,
                 mpi_pool=None):
        """
        ...
        self.__useml = useml
        self.__loadInfor = loadInfor
        self.__pert = pert
        self.__verbose = verbose
        self.__pool = mpi_pool if useMPI else get_pool(mpi=useMPI, threads=np)
        self.__scenes = self.__pool.map(initLoad, list(range(ng)))
        # a = Vector(0., Solution(self.__domain)).toListOfTuples()
        # b = escript.Tensor(0, escript.Function(self.__domain)).toListOfTuples()
        self.__disp = Vector(0., Solution(self.__domain))  # length=37 number of the nodes
        self.__volume = escript.Tensor(1, escript.Function(self.__domain))  # length=32 number of the gaussian points
        self.__strain = escript.Tensor(0, escript.Function(self.__domain))
        self.__strain_abs = escript.Tensor(0, escript.Function(self.__domain))
        self.__stress = escript.Tensor(0, escript.Function(self.__domain))
        self.__S = escript.Tensor4(0, escript.Function(self.__domain))
        self.__strain_increment = escript.Tensor(0, escript.Function(self.__domain))
        self.__stress_increment = escript.Tensor(0, escript.Function(self.__domain))
        self.__stressLast = escript.Tensor(0, escript.Function(self.__domain))
        self.__strainLast = escript.Tensor(0, escript.Function(self.__domain))
        if self.__useml:
            # from FEMxML.netTorch import Net, modelRestore
            # self.net = modelRestore(savedPath=ml_model_path, trainFlag=False)  # torch net
            if 'net' in mode:
                ...
                # -------------------------Pytorch---------------------------------
                from FEMxML.netTorchDoubleTangent import modelRestore
                ...
                #     trainFlag=False)
                self.net = modelRestore(
                    savedPath=os.path.join(ml_model_path, 'ptModelH4_30_9_Stiffness_double'),
                    trainFlag=False)
                # -------------------------Pytorch 2 net---------------------------------
    ...
    def getCurrentFabric(self, scenes):
        fabric = escript.Tensor(0, escript.Function(self.__domain))
        f = self.__pool.map(getFabric2D, scenes)
        for i in range(self.__numGaussPoints):
            fabric.setValueOfDataPoint(i, f[i])
        return fabric
    ...
    def getCurrentVoidRatio(self, scenes):
        void = escript.Scalar(0, escript.Function(self.__domain))
        e = self.__pool.map(getVoidRatio2D, scenes)
        for i in range(self.__numGaussPoints):
            void.setValueOfDataPoint(i, e[i])
        return void
    ...
    def getStrainIncrement(self):
        ...
        type: Tensor on FunctionSpace
        """
        return self.__strain_increment
    ...
    def getStressIncrement(self):
        ...
        type: Tensor on FunctionSpace
        """
        return self.__stress_increment
    ...
    def getVolume(self):
        ...
        type: Tensor on FunctionSpace
        """
        return np.average(np.array(self.__volume.toListOfTuples())[:, 0, 0])
    ...
    def solveML(self, iter_max, t):
        ...
        self.__stressLast = self.getCurrentStress()
        self.__strainLast = self.getCurrentStrain()
        ...
        # update_stress, update_s, stress_prediction = self.ml_get_stress_tangent(st=self.__strain + D,
        #                                                                         strain_abs=current_strain_abs +
        #                                                                                    self.__strain_abs)
        # update_stress, update_s, stress_prediction = self.ml_get_stress_tangent(st=self.__strain + D,
        #                                                                         stLast=self.__strainLast,
        #                                                                         stressLast=self.__stressLast)
        update_stress, update_s, stress_prediction = self.ml_get_stress_tangent(st=self.__strain + D,
                                                                                t=t)
        ...
        while True:  # only renew the u during the Iteration
            ...
            du = self.__pde.getSolution()  # pde solution for $\left (A_{ijkl}u_{k,l} \right )_{,j}=-X_{ij,j}+Y_{i}$
            try:
            ...
            update_stress, update_s, stress_prediction = self.ml_get_stress_tangent(st=self.__strain + D,
                                                                                    t=t)
            saveGauss2D(
                name=os.path.join(self.__loadInfor, 'iteration_gauss/time_%s_iter_%s.dat' % (str(t), str(iterate))),
                strain_increment=D,
                ...
                tangent=update_s,
                strain_abs=self.__strain_abs + current_strain_abs
            )
            if t >= 54:
                saveVTK(os.path.join(self.__loadInfor, "iteration_vtk/biaxialSmooth_%d.vtu" % t),
                        disp=self.__disp + u, dispIncrs=du, strain=self.__strain + D, stress=update_stress)
        """
        ...
        self.__strain += D
        self.__disp += u
        self.__strain_abs += current_strain_abs
        # a = (self.__volume*(1+trace(D))).toListOfTuples()
        self.__volume = self.__volume * (1 + trace(D))
        self.__strain_increment = D
    ...
    def solveDEM(self, iter_max=100, t=0):
        ...
        while True:  # only renew the u during the Iteration
            ...
            update_stress, update_s, update_scenes = self.applyStrain_getStressTangentDEM(st=D)  # DEM simulation
            saveGauss2D(
                name=os.path.join(self.__loadInfor, 'iteration_gauss/time_%s_iter_%s.dat' % (str(t), str(iterate))),
                strain_increment=D,
                strain_toatal=self.getCurrentStrain() + D,  # renewed total strain
    ...
    def applyStrain_getStressTangentDEM(self, st=escript.Data()):
       ...
       """
        ...
        stress = escript.Tensor(0, escript.Function(self.__domain))  # initialization stress in format of escript data
        S = escript.Tensor4(0, escript.Function(self.__domain))  # initialization tangent in format of escript data
        scenes = self.__pool.map(shear2D, list(zip(self.__scenes, st)))

        ST = self.__pool.map(getStressAndTangent2D, scenes)  # obtain the stress & tangent
        for i in range(self.__numGaussPoints):
    ...
    def ml_get_stress_tangent(self, st=escript.Data(), strain_abs=escript.Data()):
        # def ml_get_stress_tangent(self, st=escript.Data(),
        #                           stLast=escript.Data(),
        #                           stressLast=escript.Data()):
        # def ml_get_stress_tangent(self, st=escript.Data(),
        #                           t=None):

        # stLast = stLast.toListOfTuples()
        # stressLast = stressLast.toListOfTuples()
        # stLast = numpy.array(stLast).reshape(-1, 4)
        # stressLast = numpy.array(stressLast).reshape(-1, 4)
        # stLast = np.concatenate((stLast[:, 0:1], 0.5*(stLast[:, 1:2]+stLast[:, 2:3]), stLast[:, 3:4]), axis=1)
        # stressLast = numpy.concatenate((stressLast[:, 0:2], stressLast[:, 3:4]), axis=1)
        ...
        numGaussian = len(st)
        # t = np.array([t]*numGaussian).reshape([-1, 1])
        ...
        # strain_abs = np.hstack((np.hstack(
        #     (strain_abs[:, 0:1], .5 * (strain_abs[:, 1:2] + strain_abs[:, 2:3]))),
        #                         strain_abs[:, 3:4]))

        # ----------------------------torch------------------------------------
        # strainAndStrainAbs = np.concatenate((stLast, stressLast, st), axis=1)
        # strainAndStrainAbs = np.concatenate((stressLast, st), axis=1)
        strainAndStrainAbs = np.concatenate((st, strain_abs[:, 3:4]), axis=1)
        # strainAndStrainAbs = np.concatenate((st, strain_abs), axis=1)
        ...
        S = escript.Tensor4(0, escript.Function(self.__domain))  # initialization tangent in format of escript data
        # stiffness
        # stiffness_fixed = [34510619.1, -174700.62, 0.47, 16585534., -0.082, 35543959.,]
        # stiffness_fixed = [34510619.1, -174700.62, 1839013.47, 16585534., -78600.082, 35543959.,]
        stiffness_list = []
        for i in range(self.__numGaussPoints):
            ...
            if len(stiffness[0]) == 6:
                ...
                raise ValueError('stiffness length should be 6, but get %d' % (len(stiffness[0])))

            # t[0][0][0][0] = stiffness[i, 0]
            # t[1][1][0][0] = t[0][0][1][1] = 0.5 * (stiffness[i, 6] + stiffness[i, 2])
            # t[0][1][0][0] = t[1][0][0][0] = t[0][0][0][1] = t[0][0][1][0] = 0.5 * (
            #             stiffness[i, 3] + stiffness[i, 1])
            # t[1][1][1][1] = stiffness[i, 8]
            # t[1][1][0][1] = t[1][1][1][0] = t[0][1][1][1] = t[1][0][1][1] = 0.5 * (
            #             stiffness[i, 7] + stiffness[i, 5])
            # t[0][1][0][1] = t[0][1][1][0] = t[1][0][0][1] = t[1][0][1][0] = stiffness[i, 4]
