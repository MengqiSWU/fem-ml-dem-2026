...
class demConstitutive(ConstitutiveMask):
    ...
    def solver(self, deps):
        ...
        eps_abs = self.eps_abs + np.abs(deps)
        D = np.zeros([self.numg, self.ndim, self.ndim, self.ndim, self.ndim])
        r_deps = np.delete((eps_abs.reshape(self.numg, -1)), [3,6,7], axis=1)

        '''
        strain_increment Voigt Notion(re_deps):
                        0    1    2    3    4   5    
                        00   01   02   11   12  22 
                      '''
        ...
        if self.nump > 1:
            with Pool(processes=self.nump) as pool:
                ...
                # H_his = np.array(list(zip(H1, H2, H3)))
                # self.H_his = self.H_his + H_his
                # H_3D = self.H_his
