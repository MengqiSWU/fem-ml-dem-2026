...
damp = 1e6
mode = 'mldem'  # 'mldem' 'dem' 'elastic' 'csuh' 'uh' 'vonmises' 'mixed' '2ml'
if mode == 'eb':
    fric = 30.
...
while abs(u_loaded) < ly * axialStrainGoal:  # apply 100 load steps
    ...
    work_out += \
    integrate(fx_ * du_coord * [-confining, 0.] + fx * du_coord * [confining, 0.], FunctionOnBoundary(prob.domain))[0]
    x = prob.domain.getX()
    ...
    if abs(u_loaded) >= i * ly * axialStrainGoal / num_step:
        ...
        save_explicit_vtk(prob=prob, step=n, save_path=loadInfor, test_name=test_name, smooth_flag=smoothFlag, mode=mode)
    n += 1
