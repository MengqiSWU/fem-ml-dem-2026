...
test_name = 'footing_0.05'
out_directory = '../simu/%s' % test_name
mode = 'mldem'  # 'csuh' 'mldem' 'mises' 'dem' 'norsand'
surcharge = p0 = confining = 60e3  # surcharge equals to the initial vertical stress of the RVE packing
ndim = 2
# B, L, H = 0.5, 3., 1.6
B, L, H = 0.45, 3.6, 2.0
strainObject = 0.045
...
# --------------------read mesh-------------------#
# order = 1
# mshname = 'footing_tri%d' % num_mesh
# mydomain = ReadGmsh(
#     './/meshes/footing_msh_MQ/%s.msh' % mshname, numDim=ndim,
...
# --------------------generate mesh-------------------#
nx, ny = 16, 10 # sample discretization, 8 by 16 quadrilateral elements
order = 1
mydomain = Rectangle(l0=L, l1=H , n0=nx, n1=ny,
...
# ==============================================
# material properties
if mode == 'mldem':    # remember to change the history variables used
    if active_flag:
        NN_sig_path = 'active_footing_%d/X_epsANDabsxy_Y_sig_numNN3_dd5/active_0' % numg_net_arg
        NN_D_path = 'active_footing_%d/X_epsANDabsxy_Y_D_numNN3_dd8/active_0' % numg_net_arg
        ...
        NN_sig_path = 'X_epsAND3f_Y_sig_ddd14_Fourier_noRotate_640_sig'   #最终结果所用的网络
        NN_D_path = 'X_epsAND3f_Y_D_ddd14_Fourier_noRotate_640_D'
        numg_net = int(NN_sig_path.split('_')[-2])
...
p0, e0, ocr, E, poisson, lam, G, rho, nn_name, kwargs = \
    ...
    )      #rebember to change to path for different simulation (footing or biaxial)
...
# remember to change the loading path used
loadInfor = get_load_information(
    out_directory=out_directory, test_name=test_name, mode=mode, explicit_flag=False,
      nx=nx, ny=ny, order=order, numg=numg, **kwargs)

# loadInfor += '_surcharge_50e3_2.4*3.5m_2.5%strain_blk0.5_genmsh_fric1.0_400ball_p0.2_Y1.5e8'
loadInfor += '_surcharge_60e3_2.0*3.6m_4.5%strain_blk0.45_fric0.15_400ball_p0.2_Y3e8'
...
cons = getCons(mode, numg=numg, pool=None, nump=nump, explicitFlag=False, **kwargs)
