...
#### predict mesh 10*20
returned_dict_dem = get_data_series(
    root_path_list=['../../simu/Triaxial_Reld/X5_Y5_10_Y3e8_fri0.5_p0.3_rM01_denser_accum_rotation_GOOD/biax_3d_rough_implicit_dem3d_intorder1_numg2000_x5_y5_Y3e8_fri0.5_p0.3_rM01_denser_accum_rotation/iteration_gauss'],
    numg=2000)
# returned_dict_ml = get_data_series(
#     root_path_list=[
#         '../../simu/biaxial_Reld/biax_rough_implicit_mldem_intorder1_numg800_x10_y20_NNX_epsAND3f_Y_sig_ddd14_Fourier_noRotate_FEM_DEM_sig_Reld_St12_2H/iteration_gauss'],
#     numg=800)
...
def plot_case11(strain1,  stress1, indexOfPoint):
    ...
    # pyplot.xlim([-0.002, 0.092])
    # ax.set_ylim(-0.27, -0.06)
    ax.ticklabel_format(style='plain', scilimits=(-1, -1), axis='y')
    ax.set_xlabel('$\epsilon_{11}$')
    ax.set_ylabel('$\sigma_{11}$ (MPa)')

    for num in indexOfPoint:
        ax.plot(strain1[num, ::5, 0], stress1[num, ::5, 0] / 1000000, linestyle='-', color=(np.random.random(), np.random.random(), np.random.random()),
                linewidth=1.5, )    # label='$\sigma_{11}-DEM$'

    # ax.legend(loc=(0.3, 0.7), frameon=True, ncol=2)
    pyplot.savefig('./strain11_3D.png', format='png', dpi=600, bbox_inches='tight', pad_inches=0.02)
    pyplot.show()
    pyplot.close()


def plot_case12(strain1, stress1, indexOfPoint):
    ...
    # y_major_locator = MultipleLocator(0.016)
    #
    # ax.xaxis.set_major_locator(x_major_locator)  # 手动设置刻度间隔
    # ax.yaxis.set_major_locator(y_major_locator)  # 手动设置刻度间隔
    #
    # ax.set_xlim([-0.012, 0.009])
    # ax.set_ylim([-0.12, 0.064])
    ax.ticklabel_format(style='plain', scilimits=(-1, -1), axis='y')
    ax.set_xlabel('$\epsilon_{12}$')
    ax.set_ylabel('$\sigma_{12}$ (MPa)')


    for num in indexOfPoint:
        ax.plot(strain1[num, ::5, 1], stress1[num, ::5, 1] / 1000000, linestyle='-', color=(np.random.random(), np.random.random(), np.random.random()),
                linewidth=1.5, )   #label='$\sigma_{12}-DEM$'

    # ax.legend(loc=(0.2, 0.7), frameon=True, ncol=2)
    pyplot.savefig('./strain12_3D.png', format='png', dpi=600, bbox_inches='tight', pad_inches=0.02)
    pyplot.show()
    pyplot.close()



def plot_case13(strain1, stress1, indexOfPoint):
    ...
    #
    ax.ticklabel_format(style='plain', scilimits=(-1, -1), axis='y')
    # ax.set_xlim([-0.065, 0.002])
    # ax.set_ylim([-0.56, -0.08])
    ax.set_xlabel('$\epsilon_{13}$')
    ax.set_ylabel('$\sigma_{13}$ (MPa)')

    for num in indexOfPoint:
        ax.plot(strain1[num, ::5, 2], stress1[num, ::5, 2] / 1000000, linestyle='-',
                color=(np.random.random(), np.random.random(), np.random.random()),
                linewidth=1.5, )    #label='$\sigma_{22}-DEM$'



    # ax.legend(loc=(0.15, 0.75), frameon=True, ncol=2)
    pyplot.savefig('./strain13_3D.png', format='png', dpi=600, bbox_inches='tight', pad_inches=0.02)
    pyplot.show()
    pyplot.close()
...
def plot_case22(strain1,  stress1, indexOfPoint):
    ...
    for num in indexOfPoint:
        ax.plot(strain1[num, ::5, 3], stress1[num, ::5, 3] / 1000000, linestyle='-', color=(np.random.random(), np.random.random(), np.random.random()),
                linewidth=1.5, )    # label='$\sigma_{11}-DEM$'

    # ax.legend(loc=(0.3, 0.7), frameon=True, ncol=2)
    pyplot.savefig('./strain22_3D.png', format='png', dpi=600, bbox_inches='tight', pad_inches=0.02)
    pyplot.show()
    pyplot.close()
...
def plot_case23(strain1, stress1, indexOfPoint):
    ...
    # ax.set_ylim([-0.12, 0.064])
    ax.ticklabel_format(style='plain', scilimits=(-1, -1), axis='y')
    ax.set_xlabel('$\epsilon_{23}$')
    ax.set_ylabel('$\sigma_{23}$ (MPa)')


    for num in indexOfPoint:
        ax.plot(strain1[num, ::5, 4], stress1[num, ::5, 4] / 1000000, linestyle='-', color=(np.random.random(), np.random.random(), np.random.random()),
                linewidth=1.5, )   #label='$\sigma_{12}-DEM$'

    # ax.legend(loc=(0.2, 0.7), frameon=True, ncol=2)
    pyplot.savefig('./strain23_3D.png', format='png', dpi=600, bbox_inches='tight', pad_inches=0.02)
    pyplot.show()
    pyplot.close()
...
def plot_case33(strain1, stress1, indexOfPoint):
    ...
    # ax.set_xlim([-0.065, 0.002])
    # ax.set_ylim([-0.56, -0.08])
    ax.set_xlabel('$\epsilon_{33}$')
    ax.set_ylabel('$\sigma_{33}$ (MPa)')

    for num in indexOfPoint:
        ax.plot(strain1[num, ::5, 5], stress1[num, ::5, 5] / 1000000, linestyle='-', color=(np.random.random(), np.random.random(), np.random.random()),
                linewidth=1.5, )    #label='$\sigma_{22}-DEM$'
    ax.legend(loc=(0.2, 0.7), frameon=True, ncol=2)
    pyplot.savefig('./strain33_3D.png', format='png', dpi=600, bbox_inches='tight', pad_inches=0.02)
    pyplot.show()
    pyplot.close()
...
indexOfPoint = list(x[0:170])



plot_case11(strain_dem,  stress_dem,  indexOfPoint)
plot_case12(strain_dem,  stress_dem,  indexOfPoint)
plot_case13(strain_dem,  stress_dem,  indexOfPoint)
plot_case22(strain_dem,  stress_dem,  indexOfPoint)
plot_case23(strain_dem,  stress_dem,  indexOfPoint)
plot_case33(strain_dem,  stress_dem,  indexOfPoint)
