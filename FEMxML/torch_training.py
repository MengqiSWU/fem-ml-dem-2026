...
class train_class():
    ...
    def train(self, inputs, outputs, epochMax=None, optimMode='adam', epoch_Num=None):
        ...
        while noProgressNum < self.patienceNum:
            ...
            def closure():
                x_train_normed = self.model.normalization(x_train)
                outputsPredNorm = self.model(x_train_normed)
                loss = self.loss(outputsPredNorm, self.model.normalization_y(y_train))
            ...
            if epochNum % self.checkStep == 0 or epochNum == 1:  # print every 2000 mini-batches
                with torch.no_grad():
                    x_train_normed = self.model.normalization(x_train)
                    outputsNormTrain = self.model(x_train_normed)
                    lossCurrentTrain = self.loss(outputsNormTrain,
                                                 self.model.normalization_y(y_train)).item()
                    # lossCurrent = self.loss(self.model(x_validation), y_validation).item()
                    x_validation_normed = self.model.normalization(x_validation)
                    outputsNormValidation = self.model(x_validation_normed)
                    lossCurrent = self.loss(outputsNormValidation,
                                            self.model.normalization_y(y_validation)).item()
