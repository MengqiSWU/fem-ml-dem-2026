...
def get_data_3d(root_path_list,  maxTime=100, mixflag=False, explicit_flag=False, add_flag=False, series_flag=False):
    echo('Reading data...')
    ...
    stress, strain, fabric, strain_increment, tangent, strain_abs, stress_last, strain_last, H_3F, H_3D = [], [], [], [], [], [], [], [],[],[]
    H_0, H_1 = [], []
    ...
    for file in file_list:
        ...
        while i < tol:
            ...
            if 'strain_increment' in temp:
                ...
                i += (n + 1)
            elif 'strain_last' in temp or temp == 'eps_last':
                # strain_abs.append(blockDataReader(datas[i + 1:i + n + 1]))
                strain_abs += (blockDataReader(datas[i + 1:i + n + 1]))
                i += (n + 1)
            elif temp == 'sig_last':
                ...
                H_3D += (blockDataReader(datas[i + 1:i + n + 1]))
                i += (n + 1)
    ...
    stress = np.array(stress).reshape([-1, 9])
    stress = np.delete(stress, [3,6,7], axis=1)
    stress_v = stress[:,[0, 3, 5]]
    stress_r = stress[:, [1, 2, 4]]
    strain = np.array(strain).reshape([-1, 9])
    strain = np.delete(strain, [3, 6, 7], axis=1)
    # strain = np.concatenate(
    #     (strain[:, 0:1], .5*(strain[:, 1:2]+strain[:, 2:3]), strain[:, 3:4]), axis=1)
    strain_abs = np.array(strain_abs).reshape([-1, 9])
    strain_abs = np.delete(strain_abs, [3, 6, 7], axis=1)
    # strain_abs = np.concatenate(
    #     (strain_abs[:, 0:1], .5*(strain_abs[:, 1:2]+strain_abs[:, 2:3]), strain_abs[:, 3:4]), axis=1)
    tangent = np.array(tangent).reshape([-1, 21])
    index_v = [0,2,5,6,11,14,15,18,20]
    index_r = [1,3,4,7,8,9,10,12,13,16,17,19]
    D_voigt = tangent[:,index_v]
    ...
    if len(tangent)!=0:
        ...
        returned_dict['D_voigt'] = D_voigt
        returned_dict['D_rest'] = D_rest
...
def reconstruct_x_y(
        ...
        eps, sig, sigv, sigr,
        tangent=None, eps_abs=None, sig_last=None, rotate_flag=False, H_0=None, H_1=None, H_3F=None,H_3D=None, H_2D=None, D_voigt=None, D_rest=None):
    if rotate_flag:
    ...
    elif input_features == 'epsAND3d':
        input_value = np.concatenate((eps, H_3D), axis = -1)
    elif input_features == 'epsAND2d':
        input_value = np.concatenate((eps, H_2D), axis = -1)
    ...
    if output_features == 'sig':
        output_value = sig
    elif output_features == 'sigv':
        output_value = sigv
    elif output_features == 'sigr':
        output_value = sigr

    elif output_features == 'D':
        output_value = tangent
    elif output_features == 'D_voigt':
        output_value = D_voigt
    elif output_features == 'D_rest':
        output_value = D_rest
...
def sampling_index_3D(x_data, sample_num=None, ratio=None, iteration=None, numg=None, manual=False):
    n = len(x_data)
    if sample_num:
        num = sample_num
        sample_index = np.random.permutation(range(n))[:num]
    if ratio and 0. < ratio <= 1.:
        if manual == True:
            num1 = numg * iteration
            num2 = int((n-num1)*ratio)
            num = num1 + num2
            index1 = np.random.permutation(range(num1))
            index2 = np.random.permutation(range(num1, n))[:num2]
            sample_index = np.hstack((index1, index2))
        else:
            num = int(ratio * n)
            sample_index = np.random.permutation(range(n))[:num]
    else:
