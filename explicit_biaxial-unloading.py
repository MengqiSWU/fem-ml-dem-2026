...
from esys.escript import kronecker, FunctionOnBoundary, Function, \
    ...
    sqrt, inf, Vector, Solution, grad,interpolate, integrate
...
from utilSelf.general import echo, check_mkdir, getCons, get_pool, writeLine, \
...
smoothFlag = False
damp = 1e6
mode = 'mixed'  # 'mldem' 'dem' 'elastic' 'csuh' 'uh' 'vonmises' 'mixed'
lx = 0.5
...
order = 1
integration_order = 1
# mydomain is the solution domain
# mydomain = Rectangle(
#     l0=lx, l1=ly, n0=nx, n1=ny,
#     order=order, integrationOrder=integration_order) # , integrationOrder=-1) inf(mydomain.getSize())
# nn_name = 'active_X_epsANDH_Y_sigANDH_numNN3_dmdd20/active_0'
nn_name = 'X_epsANDabsxy_Y_sig_dd20_Fourier_noRotate_von_all_2'
...
save_flag = True if mode == 'dem' or mode == 'csuh' or mode == 'mldem' else False
...
p0, e0, ocr, E, poisson, lam, G, rho, nn_name, kwargs = \
    explicit_material_constants(nn_name=nn_name)
...
loadInfor = get_load_information(
    out_directory=out_directory, test_name=test_name, mode=mode, smooth_flag=smoothFlag, explicit_flag=explicitFlag,
    e0=e0, p0=p0, ocr=ocr, vel=vel, nx=nx, ny=ny, order=order, numg=numg,
    safety_coefficient=safety_coefficient, mesh_name=mesh_name, nn_name=nn_name_temp, damp=damp)
...
while abs(u_loaded) < ly*axialStrainGoal:  # apply 100 load steps
    ...
    work_out += integrate(fx_*du_coord*[-confining, 0.]+fx*du_coord*[confining, 0.], FunctionOnBoundary(prob.domain))[0]
    x = prob.domain.getX()
    ...
    if abs(u_loaded) >= i*ly*axialStrainGoal/num_step:
        ...
        writeLine(fname=fname,
                  s='%.6f %.1f %.6f %.6f %.5f %d %.3f %.3f %.3f %.3f' %\
                    (axialStrainCurrent, forceTop[1], lengthTop, (total_volume_strain), aMax, 0, ke, pe, work, work_out) + '\n',
                  mode='a')
        echo('Loading step # %d \taxialCurrentStrain: %e/%e  time_increment %e, Topforce: %.3e maxA: %.3e Time consuming: %.2e mins' %
