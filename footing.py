...
from esys.escript import whereZero, FunctionOnBoundary, interpolate, kronecker, Vector, Solution, matrix_mult, sup, \
    integrate, symmetric, trace, sqrt, inner, ReducedSolution, Data, whereNonPositive, wherePositive
...
vel = -0.00025
surcharge = -20.e3  # surcharge equals to the initial vertical stress of the RVE packing
dim = 2
B = 0.05
L = 0.6
H = 0.4
...
Dbc = whereZero(x[0]) * [1, 0] + whereZero(x[0] - sup(x[0])) * [1, 0] + whereZero(x[1] - inf(x[1])) * [1,
                                                                                                       1] + whereZero(
    x[1] - sup(x[1])) * whereNonPositive(x[0] - B) * [1, 1]
Vbc = whereZero(x[0]) * [0, 0] + whereZero(x[0] - sup(x[0])) * [0, 0] + whereZero(x[1] - inf(x[1])) * [0,
                                                                                                       0] + whereZero(
    x[1] - sup(x[1])) * whereNonPositive(x[0] - B) * [0, vel]
