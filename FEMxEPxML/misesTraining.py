...
class Net(nn.Module):
    def __init__(self, xmin, xmax, ymin, ymax, xmean, xstd, ymean, ystd, device,
                 activation=nn.Sigmoid(), inputNum=4, outputNum=1, layerList='dmddmd', node=100,
                 fourier_features=True, m_freqs=100, sigma=10., minmaxFlag=False):
        ...
        self.device = device
        self.minmaxFlag = minmaxFlag
        self.xmin, self.xmax, self.ymin, self.ymax, \
        self.xmean, self.xstd, self.ymean, self.ystd = \
            torch.tensor(xmin, dtype=torch.float, device=device), \
    ...
    def minmax(self, x, xmin, xmax, reverse=False):
        if reverse:
    ...
    def normalization(self, x, xmean, xstd, reverse=False):
        if reverse:
            normed = x * xstd + xmean
        else:
            normed = (x - xmean) / xstd
    ...
    def forward(self, x):
        # preprocess
        if self.minmaxFlag:
            x = self.minmax(x, self.xmin, self.xmax)
        else:
            x = self.normalization(x, self.xmean, self.xstd)
        ...
        # reverse process
        if self.minmaxFlag:
            y = self.minmax(x, self.ymin, self.ymax, reverse=True)
        else:
            y = self.normalization(x, self.ymean, self.ystd, reverse=True)
        return y
...
class DenseResNet(nn.Module):
    ...
    def __init__(self, xmin, xmax, ymin, ymax, xmean, xstd, ymean, ystd, device,
                 ...
                 fourier_features=True, m_freqs=100, sigma=10, tune_beta=False, minmaxFlag=False):
        ...
        self.xmin, self.xmax, self.ymin, self.ymax = torch.tensor(xmin, dtype=torch.float, device=device), \
                                                     ...
                                                     torch.tensor(ymin, dtype=torch.float, device=device), \
                                                     torch.tensor(ymax, dtype=torch.float, device=device)
        self.xmean, self.xstd, self.ymean, self.ystd = torch.tensor(xmean, dtype=torch.float, device=device), \
                                                     torch.tensor(xstd, dtype=torch.float, device=device), \
                                                     torch.tensor(ymean, dtype=torch.float, device=device), \
                                                     torch.tensor(ystd, dtype=torch.float, device=device)
    ...
    def forward(self, x):
        ...
        if self.minmaxFlag:
            out = self.minmax(out, self.ymin, self.ymax, reverse=True)
        else:
            out = self.normalization(out, self.ymean, self.ystd, reverse=True)
        return out
...
class modelTrainning:
    def __init__(self, model, savedPath, device, optimizerSTR='adam',
                 patienceNum=20, epochMax=100000, batchSize=1024, dyWeight=1.0, num_batches=10):
        self.model = model
    ...
    def trainModel(self, x, y, dy):
        ...
        # data normalization is cancelled
        x_normed = x
        y_normed = y
        dy_normed = dy
        ...
        while True:
            ...
            if epoch % self.verboseInterval == 0:
                loss = (loss0 / np.mean(np.abs(self.y_std)) ** 2 +
                        loss1 / np.mean(np.abs(self.dy_std)) ** 2) / len(x_normed)
                ...
                info = "Epoch: %d \t lr: %.3e \t Loss: %.3e/%.3e \t Loss0: %.3e \t Loss1: %.3e \t %s \ttimeConsumed: %.3e mins\n" % \
                       (epoch, self.optimizer.param_groups[0]['lr'], loss, minLoss, loss0, loss1, message,
...
def getH(epsPlastic):
    H = 0.5e9 * 0.2 * (0.05 + epsPlastic) ** 0.2
    return H


def get_dHdEps(epsPlastic):
    ''' -self.A * self.n * (self.epsilon0 + epsPlastic) ** (self.n - 1.) '''
    dHdEps = 0.5e9 * 0.2 * 0.2 * (0.05 + epsPlastic) ** (0.2 - 1.)
    return dHdEps
...
class Restore:
    def __init__(self, savedPath, device=torch.device('cuda')):
        self.device = device
        self.savedPath = savedPath
    ...
    def evaluation(self, x, y, dy):
        y_origin, dy_origin = self.prediction2(x)
    ...
    def prediction2(self, x):
        '''
            Calculating the output and the 1st differentiation of the output
        '''
        x_normed = torch.tensor(x, dtype=torch.float, requires_grad=True).to(self.device)
...
def dataReader(root_path, filePath='./MCCData/results', readContent='sig'):
    ...
    if 'sig' in readContent:
        sig = data[:, :3]
        dmisesdsig = np.array([dMisesdSig(i) for i in sig]).reshape(-1, 3)
        # mises_cal = getMises(sig).reshape(-1, 1)
        mises = data[:, 6:7]
        saveScalar(x=sig, y=mises, dy=dmisesdsig, root_path=root_path)
        ...
        epsPlastic = data[:, 7:8]
        H = data[:, 8:9]
        # H_cal = getH(epsPlastic).reshape(-1, 1)
...
if __name__ == "__main__":
    ...
    if 'mises' in trainContent:
        x, y, dy = dataReader(root_path=savedPath)
    else:
        x, y, dy = dataReader(root_path=savedPath, readContent='epsPlastic')

    sampleNum = int(0.2*len(x))
    index_random = np.random.permutation(range(len(x)))
    x, y, dy = x[index_random[:sampleNum]], \
                             y[index_random[:sampleNum]], dy[index_random[:sampleNum]]
    ...
    if trainFlag:
        # model training
        if residualFlag:
            misesNet = DenseResNet(xmin=x_min, xmax=x_max, ymin=y_min, ymax=y_max,
                xmean=x_mean, xstd=x_std, ymean=y_mean, ystd=y_std,
                device=device,
                inputNum=len(x[0]), outputNum=len(y[0]), fourier_features=fourier_features, minmaxFlag=False).to(device=device)
        else:
            misesNet = Net(
                xmin=x_min, xmax=x_max, ymin=y_min, ymax=y_max,
                xmean=x_mean, xstd=x_std, ymean=y_mean, ystd=y_std,
                device=device, activation=nn.Sigmoid(), inputNum=len(x[0]), outputNum=len(y[0]),
                layerList=layerList, node=100, fourier_features=fourier_features,
                minmaxFlag=minmaxFlag).to(device=device)
    ...
    # restore
    model_evaluation = Restore(savedPath=savedPath, device=device)
